...
def  report_pdf
  ...
  def new
    ...
    if params[:accion]
        ...
        @fuente = SourceParent.where(admin_user: current_user.admin_user).where(codigo:"EP").first.id
        @tipot =  Source.where(admin_user: current_user.admin_user).where(codigo:"GC").first
        @tipo = @tipot.id
        @evi =  @tipot.evidencia
